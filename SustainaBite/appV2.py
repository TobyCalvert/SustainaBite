...
# 1. Start a form to group inputs and the submit button
with st.form("add_item_form"):
    ...
    with col1:
        ...
        item_to_add = st.selectbox(
            label="Search Food Item:", # We must keep the label here for the code, but we want it blank to use the explicit label above. A blank label is not good.
            label_visibility="collapsed", # This is the magic! It hides the built-in label, so our st.write label is the only one visible and everything aligns.
            options=db_options
        )
    ...
    with col2:
        ...
        st.write("")
        st.write("")
        is_staple = st.checkbox("♾️ Infinite Supply", help="Tick this for staples like salt or oil that won't run out.")
    ...
    with col4:
        ...
        # We'll use the date_input's built-in label this time as it works for this specific layout.
...
if add_btn:
    if item_to_add:
        ...
        # Handle the date string safely based on the checkbox
        ...
        save_pantry(pantry_items)  # Make sure your save function is called!
        st.success(f"Added {qty if not is_staple else '∞'}x {item_to_add} to your pantry!")

        tip_found = False

        for tip in zero_waste_tips:
            # Check if any of the keywords for this tip match the item we just added
        ...
        # If no tip was found in the JSON for this item, just do a quick pause
        if not tip_found:
            time.sleep(1)

        st.rerun()

# --- NEW: End of Life / Waste Resolution UI ---
# Find all items where the days_left is less than 0
today = date.today()
expired_items = []
...
# If there are expired items, force the user to resolve them!
if expired_items:
    ...
    for idx, item in expired_items:
        ...
        with col_bin:
            if st.button("🗑️ Threw in Bin", key=f"bin_{idx}"):
                ...
                est_kg_per_serving = RF_calcsV3.estimate_serving_kg(item['name'])
...
if not pantry_items:
    ...
    for item in pantry_items:
        ...
        display_data.append({
            ...
            "Quantity": qty_str,  # Perfectly safe string
            "Expiry Date": display_date,  # Perfectly safe string
            "Days Left": delta_str,  # Perfectly safe string
            "Status": status,
            "_raw_name": item['name']
        })
...
with col_inc:
    selected_tags = st.multiselect(
        "✅ Must Include",
        options=available_tags, # Replace with whatever your tags list variable is named
...
# Optional: Add a quick safety check so they don't include and exclude the same word!
...
left_co, mid_co, right_co = st.columns([1, 4, 1])

with mid_co:
    # 2. Use a Markdown header for better visibility in documents
    st.markdown("#### Minimum Recipe Rating ⭐")

    user_min_rating = st.slider(
        label="Select Threshold",  # Keep label simple or hidden
        label_visibility="collapsed",  # Hides the default small label
        min_value=0.0,
        max_value=5.0,
        value=0.0,
        step=0.5,
        help="Higher % = more accuracy, fewer options."
    )
...
with mid_co:
    # 2. Use a Markdown header for better visibility in documents
    st.markdown("#### Minimum Number of Ingredients")
...
def draw_meal_card(meal_title, recipe_data):
    ...
    with st.expander(f"View Recipe & Data ({recipe_data['total_co2']} kg CO2e)"):
        ...
        # Check if the AI predicted this score, or if humans rated it
        if is_predicted:
        ...
        # --- NEW: Categorize Ingredients by Actionable State ---
        in_pantry = []
        to_buy = []
        ...
        for matched in recipe_data.get('matched_items', []):
            if "PANTRY" in matched or "EXPIRING" in matched or "STAPLE" in matched or "SWAP" in matched:
                ...
                # The format is "raw_name -> clean_name", so we split it to get the raw name.
        ...
        with col_ing:
            ...
            if not to_buy:
                ...
                for item in to_buy:
                    ...
                    # --- UPGRADED: Smart Substitution Logic for ALL missing items ---
                    if w2v_model:
                        clean_unmatched = RF_calcsV3.standardize_ingredient(item['search_term'])

                        if clean_unmatched in w2v_model.wv:
                            similars = w2v_model.wv.most_similar(clean_unmatched, topn=15)

                            pantry_std = [RF_calcsV3.standardize_ingredient(p.get("name", "")) for p in pantry_items]
...
with tab_7day:
    ...
    if st.button("Generate 7-Day Plan", use_container_width=True, type="primary"):
        with st.spinner("Optimizing 7-day menu..."):
            plan_7d = RF_calcsV3.generate_meal_plan(pantry_items, decimal_threshold, selected_tags, excluded_tags, user_min_ingredients,
...
with tab_1day:
    ...
    if st.button("Generate 1-Day Plan", use_container_width=True, type="primary"):
        with st.spinner("Optimizing today's menu..."):
            plan_1d = RF_calcsV3.generate_meal_plan(pantry_items, decimal_threshold, selected_tags, excluded_tags, excluded_tags, user_min_ingredients,
...
with tab_impact:
    ...
    # We use inverse delta color so that PRODUCING methane shows up in Red!
    m2.metric(
