...
# 1. Identify which recipes have a 0 average rating (meaning they had no reviews)
missing_mask = final_data['average_rating'] == 0

print(f"Predicting AI ratings for {missing_mask.sum()} unrated recipes...")

# 2. Use the X dataframe we already built to predict the missing scores!
predicted_ratings = model.predict(X[missing_mask])

# 3. Cap the predictions between 1.0 and 5.0 so we don't get weird outliers
predicted_ratings = np.clip(predicted_ratings, 1.0, 5.0)

# 4. Fill the gaps in our final dataset
...
# Convert the messy strings back into pure Python lists
...
# 7. SAVE THE ULTIMATE FILE FOR THE APP
...
def plot_xgboost_evaluation(y_test, y_pred, xgb_model, X_test):
    # Set the style for academic-looking graphs
    ...
    axes[0].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], 'k--', lw=2, color='#E74C3C')
    ...
    # --- GRAPH 2: Residuals vs. Predicted (Error Distribution) ---
    ...
    # This grabs the top 10 most important features the model used to make decisions
    ...
    # Calculate and print metrics for your report
...
# Call the function
plot_xgboost_evaluation(y_test, y_pred, xgb_model, X_test)